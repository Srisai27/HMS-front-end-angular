...
export class PatientFormComponent {
  ...
  public buildPatientForm(patient?: any): void {
    ...
    this.patientForm = this.fb.group({
      ...
      doj: [dojValue ? formatDate(patient?.doj, 'yyyy-MM-dd', 'en-US') : '', [Validators.required]],
  ...
  public addOrUpdatePatient(): void {
    ...
    if (this.patient) {
      this.dataService.updatePatient({ ...this.patientForm.value, p_id: this.patient.p_id }).subscribe(resp => {
        if (resp && resp?.status) {
          this.activeOffcanvas.close(resp?.status);
        }
      });
    } else {
      this.dataService.addNewPatient(this.patientForm.value).subscribe(resp => {
        if (resp && resp?.status) {
          this.activeOffcanvas.close(resp?.status);
        }
      });
