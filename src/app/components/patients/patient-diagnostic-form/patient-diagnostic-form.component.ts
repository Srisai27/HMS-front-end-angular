...
export class PatientDiagnosticFormComponent {
  ...
  search: OperatorFunction<string, readonly string[]> = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map((term) => {
        ...
        const filteredDiagnosis = this.diagnosis.filter((m: any) => selectedDiagnosis.every((sm: any) => sm.d_id !== m.d_id));

        return term === ''
          ? filteredDiagnosis
          : filteredDiagnosis.filter((v: any) => v.d_name.toLowerCase().indexOf(term.toLowerCase()) > -1)
      }),
    );
  };
  ...
  public addNewDiagnostic(diagnosis: any) {
    this.selectedDiagnosticsArray.push(this.fb.group({
